...
public static class Mail
{
    ...
    public static async Task Send(MailStruct ms)
    {
        ...
        message.Body = new TextPart(ms.IsBodyHtml ? "html" : "plain") { Text = ms.Body };
        ...
        await client.ConnectAsync(config.mail.smtpHost, config.mail.smtpPort, MailKit.Security.SecureSocketOptions.StartTls);
        client.SslProtocols = SslProtocols.Tls12 | SslProtocols.Tls13; // 只允许 TLS 1.2 和 1.3
        await client.AuthenticateAsync(config.mail.username, config.mail.password);
