...
namespace KanonBot
{
    public static class BotCmdHelper
    {
        public struct BotParameter
        {
            ...
            public string osu_username,
                osu_mods,
                match_name,
                search_arg; //用于获取具体要查询的模式，未提供返回osu
            public long osu_user_id,
                bid;
            public int order_number; //用于info的查询n天之前、pr，bp的序号，score的bid，如未提供则返回0
            public bool special_version_pp; //是否获取lazer数据
            public bool special_panel;
            public bool dev_panel;
            public bool sb_server;
        ...
        public static BotParameter CmdParser(
            string? cmd,
            FuncType type,
            bool parsearg1 = true,
            bool parsearg2 = true,
        ...
        )
        {
            ...
            if (!String.IsNullOrEmpty(cmd))
            {
                ...
                if (!string.IsNullOrWhiteSpace(arg5))
                {
                    ...
                    if (special_flag == "sb")
                    {
                        param.sb_server = true;
                        param.special_version_pp = true;
                    }

                    if (special_flag == "p")
                    {
                        param.special_panel = true;
                    }

                    if (special_flag == "dev")
                    {
                        param.dev_panel = true;
                    }
                }
            ...
            else
            {
                ...
                else if (type == FuncType.Score)
                {
                    ...
                    type == FuncType.Recent
                    || type == FuncType.PassRecent
                    || type == FuncType.BestPerformance
                )
                {
