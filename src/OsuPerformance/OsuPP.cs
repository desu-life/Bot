        ...
        for (int i = 0; i < accs.Length; i++)
        {
            ...
            if (rmode is Mode.Osu)
            {
                ...
                if (dattrosu is not null) {
                    if (score.IsLazer) {
                        if (mods.Contains("CL")) {
                            c.SliderTickMiss = dattrosu.Value.n_large_ticks - state.osu_large_tick_hits;
                        } else {
                            c.SliderTickMiss = dattrosu.Value.n_large_ticks + dattrosu.Value.n_sliders - state.osu_large_tick_hits;
                        }
                    }
                }
            }
